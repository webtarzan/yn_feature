...
def lag_diff(df, key_col, date_col, feature_col, lag=1, diff_order=1):
    """
    对时间序列的一个特征进行差分和滞后，并将结果与原始数据框拼接。
    df:数据集
    key_col:主键
    date_col:日期列
    feature_col:时序特征列
    ...
    # 差分
    df['diff_{}_{}'.format(diff_order, feature_col)] = df[feature_col].diff(diff_order)
    
    # 滞后
    df['lag_{}_{}'.format(lag, feature_col)] = df[feature_col].shift(lag)
    
    # 拼接
    new_cols = df[[key_col, date_col, 'diff_{}_{}'.format(diff_order, feature_col), 'lag_{}_{}'.format(lag, feature_col)]]
    
    return pd.merge(df, new_cols, on=[key_col, date_col], how='left')
...
lag_diff(df, 'cst_id', 'dt', 'x_num_01')
...
def generate_lag_features(df, key_col, date_col, cat_col, lag):
    """
    分类特征必须提前编码
    df:数据集
    key_col:主键
    ...
    """
    df = df.sort_values([key_col, date_col])
    df['lag_{}_{}'.format(lag, cat_col)] = df.groupby(key_col)[cat_col].shift(lag)
    return df
...
def arithmetic_operation(df:pd.DataFrame, col1, col2):
    """
    对两个数值特征进行四则运算衍生出新的特征
    ...
    # 加法
    df[col1 + '_' + col2 + '_add'] = df[col1] + df[col2]

    # 减法
    df[col1 + '_' + col2 + '_sub'] = df[col1] - df[col2]

    # 乘法
    df[col1 + '_' + col2 + '_mul'] = df[col1] * df[col2]

    # 除法
    df[col1 + '_' + col2 + '_div'] = df[col1] / (df[col2] + 0.00001)
...
def binary_polynomial_features(col_names: list, degree: int, features: pd.DataFrame):
    ...
    df_out = pd.concat([features, features_new], axis = 1)
    return df_out
...
arithmetic_operation(df, 'x_num_01', 'x_num_02')


# In[41]:


binary_polynomial_features(['x_num_01', 'x_num_02'], 3, df)
...
def cate_col_name(transformer, category_cols, skip_binary: bool = False):
    """
    离散字段One-Hot编码后，创建字段名
    :param transformer:
    :param category_cols:
    :param skip_binary: 是否跳过二分类的字段
    :return:
    """
    cate_cols_new = []
    ...
    for i, j in enumerate(category_cols):
        if skip_binary and (len(col_value[i]) == 2):
...
binary_cross_combination('cst_id', ['x_cat_01', 'x_cat_02'], df)
